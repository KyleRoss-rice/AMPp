...
double cSceneImitateGrav::CalcRewardImitate(const cSimCharacter& sim_char, const cKinCharacter& kin_char) const
{
	double pose_w = 0.5;
	double vel_w = 0.05;
	...
	reward = pose_w * pose_reward + vel_w * vel_reward + end_eff_w * end_eff_reward
		+ root_w * root_reward + com_w * com_reward + 0.1 * root_pos0.y();

	return reward;
}

cSceneImitateGrav::cSceneImitateGrav()
{
...
void cSceneImitateGrav::ParseArgs(const std::shared_ptr<cArgParser>& parser)
{
...
double cSceneImitateGrav::CalcReward(int agent_id) const
{
...
const std::shared_ptr<cKinCharacter>& cSceneImitateGrav::GetKinChar() const
{
	return mKinChar;
}

void cSceneImitateGrav::EnableRandRotReset(bool enable)
{
	mEnableRandRotReset = enable;
}

bool cSceneImitateGrav::EnabledRandRotReset() const
{
	bool enable = mEnableRandRotReset;
	return enable;
}

cSceneImitateGrav::eTerminate cSceneImitateGrav::CheckTerminate(int agent_id) const
{
...
std::string cSceneImitateGrav::GetName() const
{
	return "ImitateGrav";
}

void cSceneImitateGrav::ParseKinCtrlParams(const std::shared_ptr<cArgParser>& parser, cKinCtrlBuilder::tCtrlParams& out_params) const
{
...
bool cSceneImitateGrav::BuildCharacters()
{
...
void cSceneImitateGrav::CalcJointWeights(const std::shared_ptr<cSimCharacter>& character, Eigen::VectorXd& out_weights) const
{
...
bool cSceneImitateGrav::BuildController(const cCtrlBuilder::tCtrlParams& ctrl_params, std::shared_ptr<cCharController>& out_ctrl)
{
...
bool cSceneImitateGrav::BuildKinCharacter()
{
...
bool cSceneImitateGrav::BuildKinController()
{
...
void cSceneImitateGrav::UpdateCharacters(double timestep)
{
	UpdateKinChar(timestep);
	cRLSceneSimChar::UpdateCharacters(timestep);
}

void cSceneImitateGrav::UpdateKinChar(double timestep)
{
	const auto& kin_char = GetKinChar();
...
void cSceneImitateGrav::ResetCharacters()
{
	cRLSceneSimChar::ResetCharacters();
    auto grav = cMathUtil::RandDouble(-4.8, -14.8);
    mWorld->SetGravity(Eigen::Vector4d(0,grav,0,0));
	ResetKinChar();
	if (EnableSyncChar())
	{
		SyncCharacters();
...
void cSceneImitateGrav::ResetKinChar()
{
...
void cSceneImitateGrav::SyncCharacters()
{
	const auto& kin_char = GetKinChar();
...
bool cSceneImitateGrav::EnableSyncChar() const
{
	const auto& kin_char = GetKinChar();
	return kin_char->HasController();
}

void cSceneImitateGrav::InitCharacterPosFixed(const std::shared_ptr<cSimCharacter>& out_char)
{
	// nothing to see here
}

void cSceneImitateGrav::InitJointWeights()
{
	CalcJointWeights(GetCharacter(), mJointWeights);
}

void cSceneImitateGrav::ResolveCharGroundIntersect()
{
...
void cSceneImitateGrav::ResolveCharGroundIntersect(const std::shared_ptr<cSimCharacter>& out_char) const
{
	cRLSceneSimChar::ResolveCharGroundIntersect(out_char);
}

void cSceneImitateGrav::SyncKinCharRoot()
{
	const auto& sim_char = GetCharacter();
...
void cSceneImitateGrav::SyncKinCharNewCycle(const cSimCharacter& sim_char, cKinCharacter& out_kin_char) const
{
	if (mSyncCharRootRot)
	{
		double sim_heading = sim_char.CalcHeading();
...
double cSceneImitateGrav::GetKinTime() const
{
	const auto& kin_char = GetKinChar();
	return kin_char->GetTime();
}

bool cSceneImitateGrav::CheckKinNewCycle(double timestep) const
{
...
bool cSceneImitateGrav::HasFallen(const cSimCharacter& sim_char) const
{
...
bool cSceneImitateGrav::CheckRootRotFail(const cSimCharacter& sim_char) const
{
	const auto& kin_char = GetKinChar();
...
bool cSceneImitateGrav::CheckRootRotFail(const cSimCharacter& sim_char, const cKinCharacter& kin_char) const
{
...
double cSceneImitateGrav::CalcRandKinResetTime()
{
	const auto& kin_char = GetKinChar();
